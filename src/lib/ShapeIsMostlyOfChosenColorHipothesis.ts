...
export class ShapeIsMostlyOfChosenColorHipothesis extends AbstractHypothesis {
  private readonly _displayText: string = "são em maioria da cor";
  private readonly _id: string = "is-mostly-of-chosen-color";
  ...
  validate(
    ...
    const correct: boolean = chosenShapesOfChosenColor / chosenShapeTotal >= 0.4;
    const message: string = `A hipótese de que ${
      ENTITY_SHAPE_TRANSLATIONS[shape]
    }s são majoritariamente da cor ${ENTITY_COLOR_TRANSLATIONS[color]} está ${
